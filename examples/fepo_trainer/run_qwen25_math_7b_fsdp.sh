#!/usr/bin/env bash
# FEPO | Qwen2.5-Math-7B-Instruct | experimental single-GPU LoRA trainer | NVIDIA GPUs
...
OUTPUT_DIR=${OUTPUT_DIR:-checkpoints/fepo/dapo/qwen25_math_7b/${RUN_TIMESTAMP}}

PROJECT_NAME=${PROJECT_NAME:-failure-escape}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen25-math-7b-fepo-dapo-fsdp-${RUN_TIMESTAMP}}
...
TRAIN_PROMPT_BATCH_SIZE=${TRAIN_PROMPT_BATCH_SIZE:-1}
...
VLLM_CLIENT_CONCURRENCY=${VLLM_CLIENT_CONCURRENCY:-2}
LOSS_MICRO_BATCH_SIZE=${LOSS_MICRO_BATCH_SIZE:-4}
CONSOLE_SAMPLE_COUNT=${CONSOLE_SAMPLE_COUNT:-3}
########################### end user-adjustable ###########################

########################### launch ###########################
python3 -m verl.experimental.fepo.main_fepo \
    --config-name fepo_qwen25_math_7b \
    model_id="$MODEL_PATH" \
