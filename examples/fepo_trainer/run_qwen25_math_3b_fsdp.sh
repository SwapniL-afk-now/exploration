#!/usr/bin/env bash
# FEPO | Qwen2.5-Math-3B-Instruct | experimental single-GPU LoRA trainer | NVIDIA GPUs
...
########################### user-adjustable ###########################
MODEL_PATH=${MODEL_PATH:-Qwen/Qwen2.5-Math-3B-Instruct}
...
OUTPUT_DIR=${OUTPUT_DIR:-checkpoints/fepo/dapo/qwen25_math_3b/${RUN_TIMESTAMP}}

PROJECT_NAME=${PROJECT_NAME:-failure-escape}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen25-math-3b-fepo-dapo-fsdp-${RUN_TIMESTAMP}}
...
python3 -m verl.experimental.fepo.main_fepo \
    --config-name fepo_qwen25_math_3b \
    model_id="$MODEL_PATH" \
