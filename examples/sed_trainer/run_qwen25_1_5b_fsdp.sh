#!/usr/bin/env bash
# Stable Exploration Divergence | Qwen2.5-1.5B-Instruct | FSDP LoRA GRPO
...
EVAL_DATA_DIR=${EVAL_DATA_DIR:-"${WORKSPACE_ROOT}/failure-escape-runs/data/sed_eval"}
...
# data.train_batch_size is prompts per optimizer step.
...
PPO_LOSS_COEF=${PPO_LOSS_COEF:-1.0}
CLIP_RATIO=${CLIP_RATIO:-0.2}
KL_LOSS_COEF=${KL_LOSS_COEF:-0.01}

SED_ENABLE=${SED_ENABLE:-true}
SED_ALPHA_MAX=${SED_ALPHA_MAX:-0.3}
SED_DELTA=${SED_DELTA:-3.0}
SED_EMA_GAMMA=${SED_EMA_GAMMA:-0.995}
SED_EMA_UPDATE_EVERY=${SED_EMA_UPDATE_EVERY:-5}
SED_ENTROPY_THRESHOLD_RATIO=${SED_ENTROPY_THRESHOLD_RATIO:-0.5}
SED_WARMUP_STEPS=${SED_WARMUP_STEPS:-100}
...
PROJECT_NAME=${PROJECT_NAME:-verl_sed_dapo_math}
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen25_1_5b_sed_fsdp-${RUN_TIMESTAMP}}
...
EXPLORATION=(
    actor_rollout_ref.actor.exploration.enabled=${SED_ENABLE}
    actor_rollout_ref.actor.exploration.alpha_max=${SED_ALPHA_MAX}
    actor_rollout_ref.actor.exploration.delta=${SED_DELTA}
    actor_rollout_ref.actor.exploration.ema_gamma=${SED_EMA_GAMMA}
    actor_rollout_ref.actor.exploration.ema_update_every=${SED_EMA_UPDATE_EVERY}
    actor_rollout_ref.actor.exploration.entropy_threshold_ratio=${SED_ENTROPY_THRESHOLD_RATIO}
    actor_rollout_ref.actor.exploration.warmup_steps=${SED_WARMUP_STEPS}
)
